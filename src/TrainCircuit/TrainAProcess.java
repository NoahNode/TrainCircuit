...
class TrainAProcess extends Thread {
    ...
    public void run() {   // start train Process
        ...
        while (circuitCount < 5) { // keep cycling the A track loop
            theTrack.trainA_MoveAroundToSharedTrack_First(trainName); // move around A loop
            theTrack.trainA_MoveAlongSharedTrack_First(trainName); // move along shared track
            theTrack.trainA_MoveAroundToSharedTrack_Second(trainName); // move around B loop
            theTrack.trainA_MoveAlongSharedTrack_Second(trainName); // move along shared track
            theTrack.trainA_MoveBackToBeginning(trainName); //move train back to beginning
            circuitCount++;
        }
